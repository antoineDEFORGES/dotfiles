...
stow_dotfiles() {
  ...
  if ! command_exists stow; then
    warning "stow not installed, skipping"
    return 0
  fi

  cd "$DOTFILES_DIR"

  # Find stow packages (dirs with .config or dotfiles inside)
  local packages=()
  for dir in */; do
    dir="${dir%/}"
    [[ "$dir" = "packages" ]] && continue
    [[ -d "$dir/.config" || -d "$dir/.local" ]] && packages+=("$dir")
  done

  if [[ ${#packages[@]} -eq 0 ]]; then
    warning "No stow packages found"
    return 0
  fi
